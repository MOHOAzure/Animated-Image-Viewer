...
class AnimatedLabel(QLabel):
    ...
    def load_animated_image(self, path):
        self.timer.stop()
        self.frames = []
        self.durations = []
        ...
        if path.lower().endswith('.webp'):
            ...
            self.load_other_formats(path)

    def load_webp(self, path):
        im = Image.open(path)
        try:
            while True:
                frame_duration = im.info.get('duration', 100)
                frame = im.convert('RGBA')
                qimage = QImage(frame.tobytes(), frame.width, frame.height, QImage.Format_RGBA8888)
    ...
    def load_other_formats(self, path):
        im = Image.open(path)
        try:
            while True:
                buffer = io.BytesIO()
...
class ImageViewer(QMainWindow):
    ...
    def load_images(self, directory):
        ...
        for f in os.listdir(self.current_directory):
            if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.apng', '.webp')):
                try:
    ...
    def show_image(self, index):
        if 0 <= index < len(self.image_list):
            self.stop_current_thread()  # Stop any ongoing thread
            self.current_image_index = index
            self.current_image_path = os.path.join(self.current_directory, self.image_list[index])
            
            self.label.clear_resources()  # Clear resources of the previous image
    ...
    def on_image_loaded(self, image):
        ...
        if self.current_image_path.lower().endswith('.webp') and getattr(image, "is_animated", False):
            self.label.load_animated_image(self.current_image_path)
        elif 'duration' in image.info or getattr(image, "is_animated", False):
            self.label.load_animated_image(self.current_image_path)
        else:
    ...
    def load_next_image(self):
        if len(self.image_list) > 0 and not self.load_thread.isRunning():
